...
router.get('/getLatestBids', function(req, res) {
    ...
    let pageNumStr = req.query.pageNum;
    let pageSizeStr = req.query.pageSize;
    let orderTypeStr = req.query.orderType;
    tokenId = tokenId ? tokenId : '';
    ...
    stickerDBService.getLatestBids(tokenId.toString(), address.toString(), pageNum, pageSize, orderTypeStr).then(result => {
        res.json(result);
    }).catch(error => {
        console.log(error);
        res.json({code: 500, message: 'server error'});
...
router.get('/getSelfCreateCollectible', function(req, res) {
    let selfAddr = req.query.selfAddr;
    let pageNumStr = req.query.pageNum;
    let pageSizeStr = req.query.pageSize;
    let keyword = req.query.keyword;
