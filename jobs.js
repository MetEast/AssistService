    ...
    run: function() {
        ...
        let orderForAuctionJobId = schedule.scheduleJob(new Date(now + 30 * 1000), async () => {
            ...
            stickerContractWs.events.OrderForAuction({
                ...
                let orderEventDetail = {orderId: orderInfo._orderId, event: event.event, blockNumber: event.blockNumber,
                    ...
                    logIndex: event.logIndex, removed: event.removed, id: event.id, sellerAddr: orderInfo._seller,
        ...
        let orderFilledJobId = schedule.scheduleJob(new Date(now + 60 * 1000), async () => {
            ...
            stickerContractWs.events.OrderFilled({
                ...
                let orderEventDetail = {orderId: orderInfo._orderId, event: event.event, blockNumber: event.blockNumber,
                    ...
                    royaltyFee: orderInfo._royaltyFee, tokenId: result.tokenId, price: orderInfo._price, timestamp: result.updateTime, gasFee: gasFee}
                ...
                await meteastDBService.insertOrderEvent(orderEventDetail);
                await stickerDBService.updateOrder(result, event.blockNumber, orderInfo._orderId);
                await stickerDBService.updateTokenStatus(result.tokenId, 0, orderEventDetail.orderId, result.updateTime, result.endTime, 'NEW', orderInfo._buyer, event.blockNumber, false);
        ...
        if(config.cmcApiKeys.length > 0) {
            schedule.scheduleJob('*/4 * * * *', async () => {
                let res = await fetch('https://assist.trinity-feeds.app/feeds/api/v1/price')
