...
module.exports = {
    ...
    getDidByAddress: async function(address) {
        ...
        try {
            await mongoClient.connect();
            const collection = mongoClient.db(config.dbName).collection('meteast_address_did');
            let result =  await collection.findOne({address: {$regex: new RegExp(address, 'i')}}, {"_id": 0, address: 1, did: 1, role: 1});
            return result;
        } catch (err) {
            logger.error(err);
        } finally {
    ...
    getDidByDid: async function(did) {
        let mongoClient = new MongoClient(config.mongodb, {useNewUrlParser: true, useUnifiedTopology: true});
        try {
            await mongoClient.connect();
            const collection = mongoClient.db(config.dbName).collection('meteast_address_did');
            let result =  await collection.findOne({"did.did": did}, {"_id": 0, address: 1, did: 1, role: 1});
            return result;
        } catch (err) {
            logger.error(err);
        } finally {
